...
# read data
dataFile <- "results-commitment-raw-2018-12-27_03-17-46"
...
ggplot(commitmentData, aes(x=factor(commitmentData$KeyLength), y=commitmentData$Score)) +
  ...
  labs( x="Key size",  y = "computing commitments time (msec)", fill="") 

ggplot(commitmentData, aes(x=factor(commitmentData$KeyLength), y=commitmentData$Score, fill=commitmentData$KeyLength)) +
  stat_boxplot(geom ='errorbar', color="black") +
  geom_boxplot(outlier.colour="black", outlier.shape=18, outlier.size=4, notch=FALSE, fatten=2) + 
  labs( x="Key size",  y = "Computing commitments time (sec)", fill="") 

# calculate means
(means<- round(tapply(commitmentData$Score, commitmentData$KeyLength, mean), digits=2)) 

# plot means
plotmeans(commitmentData$Score ~ commitmentData$KeyLength, digits=2, ccol="red", mean.labels=T, main="Plot of computing commitments means by key length")
